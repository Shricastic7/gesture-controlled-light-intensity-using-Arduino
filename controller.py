...
try:
    ...
    # Handle the case where 'getargspec' is not available
    if "'getargspec'" in str(e):
        # Monkey-patch inspect module for compatibility
        import functools
        inspect.getargspec = functools.partial(inspect.getfullargspec)
        board = pyfirmata.Arduino(comport)
    else:
